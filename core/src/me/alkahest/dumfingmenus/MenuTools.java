package me.alkahest.dumfingmenus;
...
public class MenuTools {
    public static final int KEY_TIME = 7; // number of frames before a key is registered again
    ...
    public static class TextureRect extends MenuObject {
        ...
        public void spriteDraw(SpriteBatch sb) {
            textureRect(sb,super.shape,this.rectTexture);
        }
    }
    /**
    ...
    public static class Button extends MenuObject {
        ...
        public void draw(ShapeRenderer sr, SpriteBatch batch, boolean debug){
            if(debug){
                ...
                if(this.isPressed){
                    textureRect(batch,super.getRect(),pressed);
                }
                else{
                    textureRect(batch,super.getRect(),unpressed);
                }
            }
        }
        /**
    ...
    public static class TextField extends MenuObject {
        ...
        public void handleHeldKeys(){
            ...
            if(this.heldKeys[Input.Keys.LEFT+1] % KEY_TIME == 0){
                this.curPos = Math.max(0,this.curPos-1);
                keyPressed = true;
            }
            if(this.heldKeys[Input.Keys.RIGHT+1] % KEY_TIME == 0){
                this.curPos = Math.min(this.sOut.length(),this.curPos+1);
                keyPressed = true;
            }
            if(this.heldKeys[Input.Keys.BACKSPACE+1] % KEY_TIME == 0){
                if(this.curPos>0){
            ...
            if(this.heldKeys[Input.Keys.FORWARD_DEL+1] % KEY_TIME == 0){
     ...
     */
    public static void shapeRect(ShapeRenderer sr, Rectangle rect){
